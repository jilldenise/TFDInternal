...
void InstantReload()
{
	...
	if (CurrentWeapon)// && CurrentWeapon->WeaponLevel != 0)
	{
		/*char buffer[64];
		sprintf_s(buffer, "%p", (void*)LocalCharacter->WeaponSlotControl);
		ZeroGUI::TextLeft((char*)buffer, SDK::FVector2D{ 350.0f, 300.0f }, ColorWhite, false);*/
		SDK::UM1WeaponRoundsComponent* roundCmp = CurrentWeapon->RoundsComponent;
		if (roundCmp)
		{
...
void DrawMenu()
{
	if (ZeroGUI::Window((char*)"DescendantInternal - UnknownCheats.me", &cfg_WindowPos, SDK::FVector2D{ 500.0f, 600.0f }, cfg_DrawMenu))
	{
		...
		if (tab == 3)
		{
			...
			ZeroGUI::Text((char*)"Timescale Hold Hotkey:");
			ZeroGUI::SameLine();
			ZeroGUI::Hotkey((char*)"Timescale Hold Hotkey:", SDK::FVector2D{ 110, 25 }, & cfg_TimeScaleHoldKey);
		}
		if (tab == 4)
		{
			if (ZeroGUI::Button((char*)"AI", SDK::FVector2D{ 120, 30 }))
			{
